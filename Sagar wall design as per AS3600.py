...
fc_prime = concrete_grades_MPa[grade_key]
phi_Nu_kN = (
        phi * (thickness_mm - 1.2 * final_eccentricity_e_mm - 2 * accidental_eccentricity_ea_mm) * (0.6 * fc_prime))
...
if reo_direction == 'yes':
    area_vertical_mm2 = area_horizontal_direction = get_reo_diameter_mm2("both directions: ")
    final_area_vertical_mm2 = final_area_horizontal_mm2 = area_vertical_mm2
# python lets you declare two variable simultaneously
else:
    area_vertical_mm2 = get_reo_diameter_mm2("in vertical direction: ")
    area_horizontal_mm2 = get_reo_diameter_mm2("in horizontal direction: ")
...
suggestion_for_user = table[table >= required_Ast_mm2].stack().dropna().head(5)
if suggestion_for_user.empty:
    print(f"None of the bars we have in the market passed please add more layers or thicker bars")
else:
    print(f"you can use the following as your starter bars for the wall that you have designed:"
          f"\n{suggestion_for_user}")
for index, value in suggestion_for_user.items():
    print(index, value)
